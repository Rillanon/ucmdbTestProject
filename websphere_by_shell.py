        ...
        try:
            ...
            for configDirPath, runtimes in runtimesByConfigDirPath.items():
                ...
                if len(runtimesByCellName) > 1:
                    ...
                    continue
                # parse cell configuration and get deployment t ype

                temp = None
                try:
                    temp = pathUtil.dirName(configDirPath)
                except AttributeError:
                    logger.error("JDS debug test - Error happens at %s" % runtimes)
                profileHomeDirPath = temp
