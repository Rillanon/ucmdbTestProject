...
class ApplicationDiscovererByJmx(jee_discoverer.HasJmxProvider,
    ...
    def __init__(self, provider, descriptorParser):
    ...
    def discoverModulesForApp(self, app):
        ...
        try:
            ...
            if jeeDescriptors:
                ...
                appDescriptor = self._getDescriptorParser().parseApplicationDescriptor(descrFile.content)
        ...
        for module in app.getModules():
            ...
            try:
                ...
                if isinstance(module, jee.WebModule):
                    try:
                        descr = self._getDescriptorParser().parseWebModuleDescriptor(configFile.content)
                    except:
                        logger.warnException('Failed to get Web module descriptor')
                    else:
                        for servlet in descr.getServlets():
                            module.addEntry(servlet)
                elif isinstance(module, jee.EjbModule):
                    try:
                        descr = self._getDescriptorParser().parseEjbModuleDescriptor(configFile.content)
                    except:
                        logger.warnException('Failed to get Ejb module descriptor')
                    else:
                        for bean in descr.getBeans():
...
class ProductInformation:
    def __init__(self, name = None, version = None, buildInfo = None):
        self.name = name
...
class ProductInfoParser(jee_discoverer.BaseXmlParser):
    ...
    def parseProductConfig(self, content):
        ...
        productEl = self._getRootElement(content)
        productElNs = productEl.getNamespace()
        version = productEl.getChildText('version', productElNs)
        buildInfo = productEl.getChildText('build-info', productElNs)
        name = productEl.getAttributeValue('name')
        return ProductInformation(name, version, buildInfo)
...
class DescriptorParser(JmsResourcesParser, jee_discoverer.BaseXmlParser):
    ...
    def parseServersInServerIndex(self, content):
        ...
        if content:
            servers = []
            serverIndexEl = self._getRootElement(content)
...
#                                      'PROXY_SERVER': self._parseSoapEnabledServer
            ...
            hostname = serverIndexEl.getAttributeValue('hostName')
            for serverEntriesEl in serverIndexEl.getChildren('serverEntries'):
                serverType = serverEntriesEl.getAttributeValue('serverType')
                serverParserMethod = serverTypeParserByType.get(serverType)
                if serverParserMethod:
                    server = serverParserMethod(serverEntriesEl)
                    if hostname and not server.address:
    ...
    def _parseServerEntries(self, serverEntriesEl):
        r'@types: org.jdom.Element -> jee.Server'
        server = jee.Server(serverEntriesEl.getAttributeValue('serverName'))
        serverEntriesElNs = serverEntriesEl.getNamespace()
        for specialEndpointsEl in serverEntriesEl.getChildren('specialEndpoints', serverEntriesElNs):
            endPointName = specialEndpointsEl.getAttributeValue('endPointName')
            if endPointName == 'SOAP_CONNECTOR_ADDRESS':
                endPointEl = specialEndpointsEl.getChild('endPoint', specialEndpointsEl.getNamespace())
                if endPointEl is not None:
                    roleWithEndpoints = server.addDefaultRole(websphere.RoleWithEndpoints())
                    server.address = endPointEl.getAttributeValue('host')
                    port = endPointEl.getAttributeValue('port')
                    try:
                        roleWithEndpoints.addEndpoint(
                                netutils.createTcpEndpoint(
                                    server.address, port)
                        )
                    except Exception, e:
                        logger.warn("Failed to create endpoint (%s, %s)" % (
                                        server.address, port))

            elif endPointName.startswith('WC_defaulthost'):
                endPointEl = specialEndpointsEl.getChild('endPoint', specialEndpointsEl.getNamespace())
                if endPointEl is not None:
                    webServerRole = server.addDefaultRole(jee.WebServerRole())
                    address = endPointEl.getAttributeValue('host')
                    port = endPointEl.getAttributeValue('port')
                    try:
                        webServerRole.addEndpoint(
                                netutils.createTcpEndpoint(address, port)
                        )
                    except Exception, e:
                        logger.warn("Failed to create endpoint (%s, %s)" % (
                                        address, port))
        # deployedApplications
        applicationElements = serverEntriesEl.getChildren('deployedApplications', serverEntriesElNs)
        if applicationElements:
            role = jee.ApplicationServerRole()
            for deployedAppliationsEl in applicationElements:
                relativePath = deployedAppliationsEl.getText()
    ...
    def _parseApplicationServerInIndex(self, serverEntriesEl):
        r'@types: org.jdom.Element -> jee.Server'
        server = self._parseServerEntries(serverEntriesEl)
        return server

    def _parseAdminServerInIndex(self, serverEntriesEl):
        r'@types: org.jdom.Element -> jee.Server'
        server = self._parseServerEntries(serverEntriesEl)
        server.addRole(jee.AdminServerRole())
        return server

    def _parseNodeAgentInIndex(self, serverEntriesEl):
        r'@types: org.jdom.Element -> jee.Server'
        server = self._parseServerEntries(serverEntriesEl)
        server.addRole(jee.AgentServerRole())
        return server

    def _parseWebServerInIndex(self, serverEntriesEl):
        r'@types: org.jdom.Element -> jee.Server'
        server = self._parseServerEntries(serverEntriesEl)
        return server
